...
// Configuration
const RESERVATION_EXPIRY_MINUTES = config.slotExpireTime || 5;
const PAYMENT_EXPIRY_EXTENSION_MINUTES = config.paymentExtensionMinutes || 5;
...
async function createPaymentIntent(reservationId, userId, idempotencyKey = null) {
  try {
    ...
    const paymentData = generateEsewaPaymentData(
      transactionUuid,
      reservation.totalPrice,
      process.env.ESEWA_PRODUCT_CODE || config.esewa.productCode
    );
    ...
    return {
      success: true,
      paymentUrl: process.env.ESEWA_PAYMENT_URL || config.esewa.paymentUrl,
      paymentData,
...
function generateEsewaSignature(message) {
  const secret = config.esewa.secretKey || process.env.ESEWA_SECRET_KEY || "8gBm/:&EnhH.1/q";
...
function generateEsewaPaymentData(transactionUuid, amount, productCode) {
  ...
  return {
    ...
    failure_url: `${config.frontendUrl}/payment/failure`,
